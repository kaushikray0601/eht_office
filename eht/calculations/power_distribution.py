...
def compute_power_params(row, project_data, asme_data):
    """
    ...
    try:
        ...
        margin_on_max_cb_size = float(project_data.restrict_cb_current) / 100  # Convert percentage to fraction
        # tracer_length = float(row['total_tracer_length'])
        tracer_length = float(row['selected_tracer']['Tracer_With_Margin'])
        tracer_A_const = float(row['selected_tracer']['A_Coeff'])
        tracer_B_const = float(row['selected_tracer']['B_Coeff'])
        tracer_C_const = float(row['selected_tracer']['C_Coeff'])
        min_amb_temp = float(project_data.min_amb_t)
        operating_temp = float(row['oper_temp'])
        vendorVoltage_Correction = float(row['selected_tracer']['Voltage_Correction_Factor'])
        design_margin = float(project_data.margin_on_tracer_lengths) / 100
        # tracer_length_with_margin = tracer_length * (1 + design_margin)
        ...
        total_tracer_length = tracer_length + no_of_circuits * float(project_data.termination_margin) / 1000
       
        outer_dia_mm = asme_data.loc[asme_data['Nominal_Pipe_Size'] == float(row['line_size']), 'Outside_Diameter_mm']
        pipe_size_mm = outer_dia_mm.iloc[0] if not outer_dia_mm.empty else 25.206 * float(row['line_size']) + 9.4852
        return {
            'uid': row['uid'],
        ...
        logging.error(f"Error in power parameter computation for UID {row['uid']}: {str(e)}")
        return None


def compute_power_distribution(project_data, circuits_data):
    """
    ...
    """
    try:
        db_to_jb_length = float(project_data.ckt_ln)  # DB to JB Cable Length
        loop_length = float(project_data.loop_ln)  # JB to JB Loop Length
