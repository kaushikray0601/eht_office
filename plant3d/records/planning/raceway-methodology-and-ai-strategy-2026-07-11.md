...
| 2026-07-12 | Stage 9 schedule UI — server-side CSV + viewer summary | **Shipped** (`d561d08`): S-1..S-4 closed (pieces/offcut, generation envelope, embedded graph-warning counts, tee-deferral line); CSV is single server-side formatter; Claude review claude-notes §22 (S-5 CSV rounding-out rides Stage 10; **Stage 10 = Tier-0 telemetry moment**) |
...
| 2026-07-12 | Stage 10 warning layer — foundation + inspector UI | **Shipped** (working tree): `raceway/warnings.py` canonical vocabulary (graph warnings normalized in, not duplicated), S-5 CSV rounding-out, opt-in `screenScaledObjects` platform flag; Claude review claude-notes §23 (N-11 helper triplication, N-12 severity ordering) |
| 2026-07-12 | Tier-0 telemetry schema (`suggestion_event`) | **Design note written** (`suggestion-telemetry-design-2026-07-12.md`) — one model, batch endpoint, taxonomy v0 over existing warnings/ortho, observation-only; ready for a one-pass implementation |
| 2026-07-13 | **Tier-0 telemetry — SHIPPED (flywheel intake live)** | Peer `telemetry` app + `SuggestionEvent` + batch endpoint; server-side domain-PK stripping enforces durable-key rule; warning lifecycle + ortho axis-lock instrumented; migration on live PG; Claude review claude-notes §26. From this date, engineering decisions are being recorded with context |
| 2026-07-12 | Solid three-face proxy (§20-a) | **Shipped**: one merged `BufferGeometry` + one mesh + one material per run (draw-call guidance honored); bottom-reference plane, catalogue-mm envelope, derived/non-persistent; Claude review claude-notes §24 — next pass recommendation: **telemetry foundation before clash**, so clash warnings are born instrumented |
| 2026-07-13 | Rough AABB clash warnings (Phase G first slice) | **Shipped, born instrumented**: 0.10 m band + 2000-object cap **with truncation disclosure**, flood cap 25, N-11/N-12 closed via `raceway/geometry.py`; Claude review claude-notes §27 — N-13 (move ModelObject query behind a plant3d seam) + `RenderTile.bounds` partition endorsed as next scalable step before BVH |
| 2026-07-13 | N-13 seam fix + tile prefilter + warning UX + shortcut audit | **Shipped** (working tree): bounds query behind `plant3d.overlay` seam with guard test; `RenderTile.bounds` prefilter live; clickable warning rows; T-1 event dictionary started. Claude §28: one stale-assertion test failure (N-14) to fix before commit; register sweep + 5 low-hanging UX items listed |
| 2026-07-13 | Fitting projection v0 (`raceway/fittings.py`) | **Shipped, projection-only**: bends/risers/reducer candidates with transition taxonomy + `requires_face_alignment` honesty statuses; persistence correctly deferred until face/orientation semantics settle. Claude §29: signals endorsed; add non-standard-bend-angle flag (N-15) + service-mismatch warning (N-16) before persistence; face/orientation work starts as a design note |
| 2026-07-13 | Face/orientation foundation — run-level presets | **Shipped** (`5773efc` + working tree): design note written first (condition honored); 4 orthogonal presets persisted as versioned, server-canonicalized run metadata (no migration). Claude §32: approved; N-17 (clash envelope must consume orientation) rides next pass; node-UUID preservation confirmed as correctness prerequisite (PUT currently regenerates keys every save) |
...
| 2026-07-16 | Segment face-offset foundation | **Shipped**: `segment_face_offset.v0` with the shared intent-persistence idiom (node-pair keys, ±5 m clamp, epsilon, stale pruning); **envelope consumes orientation + offset with regression test — N-17 lesson applied proactively**. Claude §37: approved; `face_offset_step` vocabulary rider for the reducer pass now shipped; N-18 fixture id shortened, with local PG verification still blocked by connection setup |
| 2026-07-17 | Reducer edge-match authoring action | **Shipped**: `Apply Edge Match` / `Shift+T` consumes unresolved one-edge reducer suggestions and writes recommended segment `face_offset_m` values into `segment_face_offset.v0` as one undoable draft edit; no accessory persistence yet. First suggestion-accept loop live via `raceway.reducer.edge_match_offset` shown/accepted telemetry. Browser smoke covers 300→600 mm edge-match application + accepted telemetry; real-viewer tests now self-provision catalogue rows after seed flush and use cold-start-safe readiness waits |
| 2026-07-16 | Reducer one-edge matching + offset-step signals | **Shipped, projection-only**: left-edge default with math-defined convention, per-member recommended offsets, honest `requires_face_alignment` resolution; §37 `face_offset_step` rider delivered **with T-1 dictionary row in-pass**; N-18 closed. Claude §38: approved; N-19 browser cold-start flake to fix; **suggestion-apply UX = first Tier-0 telemetry wiring moment** |
| 2026-07-17 | **Edge-match apply command — the flywheel's first turn** | **Shipped**: `Apply Edge Match` with the saved-state precondition (now doctrine for all future apply-suggestion commands); `raceway.reducer.edge_match_offset` shown/accepted telemetry live and asserted end-to-end in a real-viewer browser test; N-19 closed (45 s waits + order-dependency root cause). From this date the Tier-2 corpus accumulates real suggestion acceptances. Claude review §39 |
| 2026-07-17 | Accessory geometry doctrine note | **Written**: `raceway-accessory-geometry-note-2026-07-17.md` clarifies that edge-match is not a reducer, real accessories are port-derived parametric bodies, reducers need left/right/center handedness plus tapered/curved transition geometry, and six-direction segment movement is separate from local face offset. Next order refined: split/insert semantics → reducer handedness UI → reducer proxy geometry v0 |
| 2026-07-18 | Accessory geometry note (Codex) + Claude design review | Note **approved** (claude-notes §40): port-based accessory model, Apply-Edge-Match ≠ reducer correction, 3-phase persistence. Pre-coding requirements: **A-1 port-frame handedness convention** (must-decide), **A-2 straight-proxy cutback** (must-address); open questions answered — left_edge default no forced modal, heuristic-before-catalogue, 15° collinearity gate on reducer suggestions |
| 2026-07-18 | Segment split/insert foundation | **Shipped**: percentage split with immediate node selection, both intent kinds inherited per §33, draft remap + UUID re-keying, per-kind merge preservation. Claude §41: approved; **N-20** — agreeing face offset must not survive a merge that changes its orientation frame (offset is applied in the oriented basis; few-line fix). Next: reducer handedness UI → reducer proxy v0 (A-2 cutback) |
| 2026-07-28 | **M-3 fully realized; accessory arc closed for MVP** | Make Tee (click-on-segment split+join, one undo step) + Make Cross (unconnected-crossing warning as picker) shipped and KR-accepted; B-1..B-4 + C8 closed; A-7 decided (drafting-controls stance); graph/fitting/schedule contract pins + fail-loud validation live. Claude §46–§47. **Next: C10 module-split tail → Phase H + durable EHT persistence** |
