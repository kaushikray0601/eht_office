...
- Clean-DB fresh run on source 47/package 55 for the same `8-SSPAU-800203.ifc` sample reported about 1 minute 6 seconds conversion through `process_plant3d_job --watch`, CPU around 40%, GPU 1 around 4%, GPU 2 0% from Windows Task Manager. Viewer metrics: 4,313 objects, 9/9 loaded tiles, 715,028 triangles, 36 draw calls, 16,050,726 package bytes, 287 ms load time, 60 FPS, 18.2 ms frame time, pixel ratio 0.9, 22 MB browser heap, 5 ms pick latency, and 55 ms metadata latency. WebGL renderer was `ANGLE (NVIDIA, NVIDIA GeForce GTX 1050 Ti ... Direct3D11)`, confirming the viewer path is GPU-backed even though conversion is CPU/IO-bound.
- Interpretation after package 55: rendering/picking is no longer the bottleneck for this scale. The unresolved performance question is conversion breakdown, so new conversions now record per-stage timing (`parse_ms`, `glb_build_ms`, `tile_write_ms`, `db_write_ms`, etc.) and `measure_plant3d_package` surfaces that timing block.
- 2026-07-01 manual timing run on a fresh 13.7 MB `8-SSPAU-800203.ifc` GLB conversion recorded 61,073 ms total. The timing block was: source read 5 ms, IFC parse 59,656 ms, metadata 17 ms, tile grouping 11 ms, tile prep 72 ms, GLB build 479 ms, meshopt hook 406 ms, tile write 29 ms, tileset write 1 ms, DB/index write 329 ms. Parser threads were `1` from the default setting. Interpretation: about 97.7% of total conversion time is IfcOpenShell parse/tessellation, so the next controlled performance test should use `process_plant3d_job --watch --parser-threads auto` or a fixed worker thread count.
- 2026-07-01 parser-thread A/B run on the same 13.7 MB `8-SSPAU-800203.ifc` sample used `venv/bin/python manage.py process_plant3d_job --watch --parser-threads auto`. Total conversion improved to 11,826 ms. Timing block: source read 9 ms, IFC parse 10,411 ms, metadata 18 ms, tile grouping 11 ms, tile prep 69 ms, GLB build 473 ms, meshopt hook 389 ms, tile write 29 ms, tileset write 1 ms, DB/index write 343 ms. CPU rose to about 97%, GPU remained unchanged, and RAM stayed around 30%. Interpretation: threaded IfcOpenShell parsing is a large confirmed win on this workstation, reducing the tested conversion from about 61 s to about 12 s while preserving the same package shape.
- 2026-07-01 worker safety update: parser-thread `auto` is now Docker/cgroup-aware for CPU quota and memory limit files where Linux exposes them. Shared/small Docker hosts can additionally use `--parser-thread-cap N`, `PLANT3D_PARSER_MEMORY_PER_THREAD_MB`, and `PLANT3D_PARSER_MEMORY_RESERVE_MB`. The worker calls Python garbage collection after each job, and long-running native-memory hygiene should use `--max-jobs N` with a Docker/systemd restart policy.
- 2026-07-01 follow-up manual check on 11.9 MB `8-SSPAR-800205A.ifc` reported consistent threaded-worker behavior: 6,501 ms total, `parse_ms=5,334 ms`, 4,350 objects, 9 tiles, 6,699,712 GLB bytes, 1,513,214 sidecar bytes, and `ifcopenshell_iterator_threads=23`. Windows Task Manager did not visibly show a CPU spike, likely because the heavy parse stage lasted only about 5 seconds and the graph/sampling view averaged it away. New conversions now record process CPU time and CPU/wall ratio to prove multi-core work from job data rather than visual Task Manager sampling.
- Test cleanup note: deleting plant3d rows through Django admin is acceptable for local spike DB cleanup, but it leaves storage blobs under the configured media/storage root. `purge_plant3d_data` now provides the safer clean-test path: dry-run a scope first, then add `--confirm` to remove DB rows and explicit source/render storage blobs.
- Unit fixture update: the synthetic known-dimension coverage now includes both metre-declared and foot-declared one-metre cases. The foot-declared fixture records source length as `ft` / `0.3048` while preserving a 1.0 m rendered extent, matching the current IfcOpenShell-SI geometry contract. This does not replace a real source-system/exporter known-dimension benchmark.
