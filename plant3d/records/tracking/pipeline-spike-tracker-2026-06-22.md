...
- 2026-07-14 retention update: KR reversed the earlier disposable-working-upload policy after manual use. Normal source uploads now create/retain separate `SourceModel` rows until explicit user deletion; later uploads no longer prune prior unsaved uploads for the same user/project. The saved-case marker remains for deliberate protected references and the five-case cap, not as the only way to avoid upload replacement.
