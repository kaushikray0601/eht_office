...
class Plant3DIntakeTests(TestCase):
    ...
    def test_upload_page_auto_submits_after_file_selection(self):
        user = get_user_model().objects.create_user(username="plant3d-upload-page-user", password="pw")
        assign_project(user, self.project)
        self.client.force_login(user)

        response = self.client.get(reverse("plant3d_source_upload"))

        self.assertEqual(response.status_code, 200)
        self.assertContains(response, "plant3dSourceUploadForm")
        self.assertContains(response, "sourceUploadStatus")
    ...
    def test_package_and_tile_json_endpoints_return_render_payload(self, mock_parse):
        ...
        self.assertEqual(package_payload["project_id"], self.project.proj_id)
        self.assertEqual(package_payload["source_model_id"], source.pk)
    ...
    def test_package_viewer_page_exposes_package_api_url(self, mock_parse):
        ...
        self.assertContains(response, "20260714_snap_provider3")
    ...
    def test_package_viewer_static_js_exposes_generic_layer_registry(self):
        ...
        self.assertIn("screenScaledObjects: Boolean(config.screenScaledObjects)", content)
        self.assertIn("getMeasurementSnapObjects: config.getMeasurementSnapObjects || null", content)
        self.assertIn("function layerMeasurementSnapObjects", content)
        self.assertIn("function measurementSnapObjectsFromViewerLayers", content)
        self.assertIn("const MEASUREMENT_LAYER_SNAP_SCREEN_PX = 9", content)
        self.assertIn("function closestLineSnapPointFromObject", content)
        self.assertIn("function closestLayerMeasurementSnapPoint", content)
        self.assertIn("const layerSnapPoint = closestLayerMeasurementSnapPoint(event)", content)
        self.assertIn("function closestModelMeasurementSnapPoint", content)
        self.assertIn("const modelSnapPoint = closestModelMeasurementSnapPoint()", content)
        self.assertIn("function togglePlantModelLayerVisibility", content)
        self.assertIn("key === 'm' && event.shiftKey", content)
        self.assertIn("snapObjectCount: layerMeasurementSnapObjects(layer).length", content)
        self.assertIn("function updateScreenScaledGraphic", content)
        self.assertIn("function updateViewerLayerGraphicsScale", content)
        self.assertIn("updateViewerLayerGraphicsScale();", content)
        ...
        self.assertIn("function pushDraftHistory", content)
        self.assertIn("function undoDraftChange", content)
        self.assertIn("function redoDraftChange", content)
        self.assertIn("function pushRouteHistory", content)
        self.assertIn("function undoRouteChange", content)
        self.assertIn("function redoRouteChange", content)
        self.assertIn("function liveUpdateDraftPositionFromInput", content)
        self.assertIn("function updateRouteNodeCoordinate", content)
        self.assertIn("function deleteSelectedRouteGuide", content)
        self.assertIn("function routeReferencesDraftElement", content)
        ...
        self.assertIn("function connectableAnchorFromViewerEvent", content)
        self.assertIn("function beginRouteWorkflow", content)
        self.assertIn("function handleRouteWorkflowClick", content)
        self.assertIn("function setRouteSourceAnchor", content)
        self.assertIn("function setRouteDestinationAnchor", content)
        self.assertIn("function addRouteGuidePoint", content)
        self.assertIn("function centerlineGuideInsertIndex", content)
        self.assertIn("function routeGuideInsertIndex", content)
        self.assertIn("function distancePointToSegmentSquared", content)
        self.assertIn("function orthogonalGuideDragPoint", content)
        self.assertIn("function updateOrthogonalRouteButton", content)
        self.assertIn("function undoLastRouteGuidePoint", content)
        self.assertIn("function resetRouteGuidePath", content)
        self.assertIn("function editDraftRoute", content)
        self.assertIn("function routeMetadataPatch", content)
        self.assertIn("function routeValidationForPoints", content)
        self.assertIn("function routeWarningSummary", content)
        self.assertIn("function routeWarningBadgesHtml", content)
        self.assertIn("function routeProfileForPoints", content)
        self.assertIn("function routeProfileForElement", content)
        self.assertIn("function renderRouteHud", content)
        self.assertIn("function draftRouteValidationHtml", content)
        self.assertIn("function manhattanRouteThroughGuides", content)
        self.assertIn("function directRouteThroughGuides", content)
        self.assertIn("function refreshPendingRouteFromGuidePoints", content)
        self.assertIn("direct_guide", content)
        self.assertIn("Centerline mode", content)
        self.assertIn("Ortho Assist", content)
        self.assertIn("routeProfile", content)
        self.assertIn("manhattanSegmentPoints", content)
        self.assertIn("validateRoute", content)
        self.assertIn("suggestManhattanRoute", content)
        self.assertIn("function routeGuideHandle", content)
        self.assertIn("function renderPendingRouteGuideHandles", content)
        self.assertIn("function beginRouteGuideDrag", content)
        self.assertIn("function updateRouteGuideDrag", content)
        self.assertIn("function finishRouteGuideDrag", content)
        self.assertIn("pendingRouteGuidePoints", content)
        self.assertIn("draggingRouteGuideIndex", content)
        self.assertIn("selectedRouteGuideIndex", content)
        self.assertIn("editingRouteId", content)
        self.assertIn("ehtRedoBtn", content)
        self.assertIn("ehtDeleteGuideBtn", content)
        self.assertIn("ehtOrthogonalRouteBtn", content)
        self.assertIn("routeOrthogonalEdit", content)
        self.assertIn("routeGuideDragStartPoint", content)
        self.assertIn("ehtEditSelectedRouteBtn", content)
        ...
        self.assertIn("routeWorkflowState", content)
        self.assertIn("select_source", content)
        self.assertIn("select_destination", content)
        self.assertIn("edit_route", content)
        ...
        with open(routing_path, encoding="utf-8") as script:
            routing_content = script.read()
        self.assertIn("export function suggestManhattanRoute", routing_content)
        self.assertIn("export function manhattanSegmentPoints", routing_content)
        self.assertIn("export function routeDiagnostics", routing_content)
        self.assertIn("export function validateRoute", routing_content)
        self.assertIn("export function routeProfile", routing_content)
        self.assertIn("export function createRouteGraph", routing_content)
        self.assertIn("export function summarizeRouteGraph", routing_content)
        self.assertIn("routeWarning", routing_content)
        self.assertIn("same_source_destination", routing_content)
        self.assertIn("many_bends", routing_content)
        self.assertIn("axisOrder", routing_content)
    ...
    def test_source_detail_page_wires_conversion_polling_script(self):
        ...
        self.assertNotContains(response, "Open 3D Viewer")
        self.assertContains(response, "Save This Geometry Case")
        self.assertContains(response, "Delete This Source Model")
        ...
        self.assertContains(response, "20260714_sourceui3")
        self.assertContains(response, 'aria-label="Latest conversion progress"')
        self.assertNotContains(response, 'aria-label="Conversion progress"')
