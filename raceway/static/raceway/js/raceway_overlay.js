...
function racewayProjectionCoreFunction(name, fallback) {
  const helper = window.racewayProjectionCore?.[name];
  if (typeof helper === 'function') return helper;
  console.warn(`Raceway projection core helper missing: ${name}.`);
  return fallback;
}

function emptyScheduleSummaryViewModel() {
  return {
    runCount: 0,
    lengthM: 0,
    pieceCountEstimate: 0,
    offcutM: 0,
    planBendCount: 0,
    riserCount: 0,
    teeCount: 0,
    crossCount: 0,
    supportPlaceholderCount: 0,
    branchProjectionOnlyCount: 0,
    warningText: '',
    assumptionCount: 0,
    groupRows: [],
    hiddenGroupCount: 0,
...
function emptyFittingSummaryViewModel() {
  return {
    totalCount: 0,
    syntheticProxyCount: 0,
    planBendCount: 0,
    riserCount: 0,
    teeCount: 0,
    crossCount: 0,
    reducerCandidateCount: 0,
    reducerProxyCount: 0,
    requiresFaceAlignmentCount: 0,
    requiresCatalogueValidationCount: 0,
    nonStandardPlanBendCount: 0,
    edgeMatchCandidateCount: 0,
    faceOffsetStepCount: 0,
    faceAlignmentResolvedCount: 0,
    junctionNodeCount: 0,
    branchNodeCount: 0,
    assumptionCount: 0,
    categoryRows: [],
...
function validateGraphProjectionContract(graph) {
  return racewayProjectionCoreFunction('validateGraphProjectionContract', () => {})(graph);
}
...
function buildScheduleSummaryViewModel(schedule) {
  return racewayProjectionCoreFunction(
    'buildScheduleSummaryViewModel',
    emptyScheduleSummaryViewModel
  )(schedule);
}
...
/**
 * Delegate command availability rules to the extracted pure helper while
 * preserving the browser-test seam on window.racewayViewerOverlay.
 */
function computeRacewayCommandStates(snapshot = {}) {
  return racewayProjectionCoreFunction('computeRacewayCommandStates', () => ({}))(snapshot);
}
