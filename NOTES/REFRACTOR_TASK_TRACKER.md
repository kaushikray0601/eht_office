...
- [x] Task 10.10: Implement the first controlled split-feeder workflow: select branches/circuits to move to a new feeder path, preview generated MCB/cable/JB changes, require user confirmation, and store audit remarks.
...
- Task 10.9 first combine-feeders workflow: added a controlled MCB-source selection mode in the SLD inspector, preview/apply endpoints, next-available breaker recommendation, audit remarks, and applied `SLDTopologyEdit` persistence. This first workflow intentionally applies a graph-level feeder combine with an explicit review warning; detailed cable/JB materialization and cable-sizing changes remain future domain-edit work.
- Task 10.10/10.11 split-and-state pass: added the first controlled split-circuits workflow by selecting downstream circuit components, previewing a new manual MCB, recommending the next available breaker, and persisting an audited topology edit. Added generated-vs-edited/recalculated SLD UI state, active-edit warnings, baseline fingerprints, and a reset-to-generated endpoint/button. This remains intentionally graph-level for the first slice; detailed cable/JB materialization and cable sizing are still explicit follow-up domain work.
